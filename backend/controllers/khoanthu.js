...
export const _statistics = (req, res) => {
	db.query(
				`
SELECT khoanthu.*, 
			COALESCE(n.tong, 0) tong, 
			COALESCE(n.danop, 0) danop, 
			(SELECT COUNT(*) FROM hokhau) - COALESCE(n.danop, 0) chuanop
		FROM (
			SELECT makhoanthu, SUM(sotien) tong, COUNT(sotien) as danop
			FROM noptien
			GROUP BY makhoanthu
		...
		RIGHT JOIN khoanthu ON n.makhoanthu = khoanthu.id
		WHERE $1 = 0 OR khoanthu.id = ANY($2)
		`,
		[req.body.length, req.body],
		(error, results) => {
			if (error) return res.status(500).json(error);
